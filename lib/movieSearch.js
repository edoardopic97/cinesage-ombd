...
- For ranking or rating queries (e.g. "highest rated", "top rated", "best"), return well-known, critically acclaimed titles. You do NOT need to verify exact ranking positions.
...
- Return ONLY the JSON, no extra text.`;
  ...
  console.log("[LLM] Cleaned for parsing:", clean);
  const parsed = JSON.parse(clean);
  console.log("[LLM] Parsed movies:", JSON.stringify(parsed.movies || []));
...
async function findMoviesFromQuery(userQuery, category = "all") {
  console.log(`[Search] Query: "${userQuery}", Category: "${category}"`);
  const identified = await identifyMoviesFromQuery(userQuery, category);
  console.log(`[Search] LLM identified ${identified.length} titles`);

  const results = await Promise.all(
    identified.map(async ({ title, year, type }) => {
      try {
        const result = await searchOMDB(title, year, type);
        console.log(`[OMDB] "${title}" (${year}, ${type}) → ${result ? "found" : "not found"}${result ? `, Type: ${result.Type}` : ""}`);
  ...
  console.log(`[Search] Final results: ${final.length}/${identified.length}`);
  return final;
}
