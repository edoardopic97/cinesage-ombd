...
- Prefer real, current data such as IMDb ratings, critic scores, or reputable rankings.
...
async function directTitleSearch(title, category = "all") {
  if (!OMDB_API_KEY) throw new Error("OMDB API key not configured");
  const type = category === "movie" ? "movie" : category === "tv" ? "series" : undefined;
  const allMatches = [];

  for (let page = 1; page <= 4; page++) {
    const params = { apikey: OMDB_API_KEY, s: title, page };
    if (type) params.type = type;
    const res = await axios.get("http://www.omdbapi.com/", { params });
    if (res.data.Response !== "True" || !res.data.Search?.length) break;
    allMatches.push(...res.data.Search);
  }
  ...
  const results = await Promise.all(
    unique.map(async (m) => {
      try {
        const full = await axios.get("http://www.omdbapi.com/", { params: { apikey: OMDB_API_KEY, i: m.imdbID } });
        if (full.data.Response !== "True") return null;
        const r = full.data;
        return {
          Title: r.Title, Year: r.Year,
          Poster: r.Poster && r.Poster !== "N/A" ? r.Poster : `https://img.omdbapi.com/?apikey=${OMDB_API_KEY}&i=${r.imdbID}`,
          Genre: r.Genre, Plot: r.Plot, imdbRating: r.imdbRating, Runtime: r.Runtime,
          Country: r.Country, Type: r.Type, Director: r.Director, Actors: r.Actors,
          Language: r.Language, Awards: r.Awards !== "N/A" ? r.Awards : undefined,
          imdbID: r.imdbID, Rated: r.Rated !== "N/A" ? r.Rated : undefined, Ratings: r.Ratings,
        };
      } catch { return null; }
    })
  );
